name = "Kyklop's electronic warfare mod";
/// Picture displayed from the expansions menu/ Optimal size is 2048x1024, other sizes work too
picture = "kyk_ew\icons\Steam main 1024x1024.paa"; 
/// Display next to the item added by the mod
logoSmall = "kyk_ew\icons\ace\EW.paa";
/// Logo displayed in the main menu
logo = "kyk_ew\icons\ace\EW.paa";
/// Tool tip displayed when the mouse is left over, in the main menu of Arma 3
tooltipOwned = "Kyklop's electronic warfare mod";
/// Color used for DLC stripes and backgrounds (RGBA)
dlcColor[] = {	0.23, 0.39, 0.30, 1};
/// Overview text, displayed from the extension menu
overview = "This mod adds basic electronic warfare features into Arma 3. It simulates things like radio jamming, Radio frequency detonation jamming, Radar jamming etc. This mod is not very complex and only works with certain other mods. For example radio jamming only works with TFAR and explosives jamming only works with ACE3 compatible explosives.";
/// Hide the extension name 
hideName = 0;
/// Hide the extension menu
hidePicture = 0;
author = "KyklopCZ";