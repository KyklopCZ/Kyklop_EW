...
if(isServer) then
{
	...
	kyk_ew_tfar_jammerScriptEnabled = true;
	kyk_ew_tfar_jammerUpdateInterval = 5;
	
	while { kyk_ew_tfar_jammerMasterSwitch } do
	{
		while { kyk_ew_tfar_jammerScriptEnabled } do
		{
			...
			for "_i" from 0 to count _players - 1 step 1 do
			{
				...
				[_radioJammed, _radioJammingSignalStrength] remoteExecCall ["kyk_ew_tfar_fnc_setRadioJammed", _currentPlayer, false];
			};
			//Radio jamming end
			
			sleep kyk_ew_tfar_jammerUpdateInterval;
		};
		
		private _players = call BIS_fnc_listPlayers;
		
		{
			[false, 0] remoteExecCall ["kyk_ew_tfar_fnc_setRadioJammed", _x, false];
		} forEach _players;
		
		waitUntil {kyk_ew_tfar_jammerScriptEnabled || !kyk_ew_tfar_jammerMasterSwitch;};
	};
};